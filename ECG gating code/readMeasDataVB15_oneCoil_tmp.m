...
% try  readNavigator=OFF
% and readTimeStamp = ON    EVRD  3/30/11
...
% filename='/v/raid1/gadluru/MRIdata/Cardiac/Verio/P021411/RawData/meas_MID31_CV_Radial7Off_flexICE_rcECG_pd_reconILOT35_STRESS_FID49586.dat'
% filename='/v/raid1/gadluru/MRIdata/Cardiac/Verio/P041411/RawData/meas_MID70_CV_Radial7Off_flexICE_rcECG_pd2_reconILOT35setup_FID66772.dat'
% filename='/v/raid1/gadluru/MRIdata/Cardiac/Verio/P040111B/RawData/meas_MID267_SOS_centric_IL_tz_sparser0401_sat14deg_FID62892.dat'
% 
% filename='/v/raid1/gadluru/MRIdata/Cardiac/Verio/P041012/RawData/meas_MID221_CV_Radial7Off_golden_newSatprot_11_2ml_adeno_FID150982.dat'
...
%   timeStamp = zeros(nSlice, nRepetition, 'single');
%   tstamps=zeros(1,nTimeStamp);
end
...
tmp_timeS=[tmp_timeS timeS];
%          tStamps(indexx)=timeS;
%          indexx=indexx+1
         st = fseek(fid,4,0);
      else
         st = fseek(fid,20,0);
  ...
  %          skipField = nPhCorScan*nPhCorEcho*Ncr*(localHeader+8*Nxr)-localHeader;
       ...
       %        infoMDH_TimeStamp = readMDH_TimeStamp_VB13(fid);
          ...
          elseif readOneCoil == 1 & Channel+1 == coilIndex
             ...
             count = count+1;
             timeStamp(Slice+1, Repetition+1) = timeS;
             
          end

          
           if readTimeStamp == 1 & Channel == 0   % EVRD  3/30/11
             %EPITrain = mod(countNavigator, nEPITrain);
             %timeStamp(Echo+1,  Partition+1, Slice+1, Acquisition+1, Phase+1, Repetition+1) = single(0.0025*timeS);
             %timeStamp(Slice+1, icoil+1, Repetition+1) = timeS;  %tt=tStamps(1:4:end);   REPET. NOT HUGH ENOUGH, repeats every nrays*nslices*ncoils
           end
...
size(timeStamp)
timestamps_OneperImage_oneslice=squeeze(timeStamp(3,:));
%save test_timestamps31 timestamps_OneperImage_oneslice

%save P041411_MID70_test_timestamps timestamps_OneperImage_oneslice
%save P040111A_MID75_test_timestamps timestamps_OneperImage_oneslice
save P012712_rest timestamps_OneperImage_oneslice
