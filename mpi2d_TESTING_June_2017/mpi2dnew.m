...
if(nargin==0) % help message printout mode only
 ...
 p  = buf2par(p,arg2buf(nargin,varargin),1);
...
p  = buf2par(p,readtxt(in),5);
stg = getpar(p,'double','stg',   '1','List of stages to execute');
wordy = getpar(p,'double','wordy','0'   ,'Messages: 0-silience, 4-very wordy');
...
framesToSkip = getpar(p,'double','framesToSkip','[]','Frames to leave out due to motion, will be replaced by linear interpolation of neighbors');
delta_t = getpar(p,'double','delta_t','2','Time between frames (same slice), in seconds');
delta_t=0.5;
SRT=getpar(p,'double','SRT','80','Satration recovery time');
...
numPreContrast_bld = getpar(p,'double','numPreContrast_bld','5','self-explanoatory');
numPreContrast_tiss = getpar(p,'double','numPreContrast_tiss','6','self-explanoatory');
...
slice_satfreq=getpar(p,'double','slice_satfreq','5','Sat pulse repetition freq');
sin_a=getpar(p,'double','sin_a','10','Flip angle');
sin_asmall=getpar(p,'double','sin_asmall','4','Flip angle PD');
TD=getpar(p,'double','TD','20','Time delay in ms');
nor=getpar(p,'double','nor','15','Num of rays');
diffthld= getpar(p,'double','diffthld','1','Mask threshold, in std units'); %#ok<NASGU>
...
flag_PD_end= getpar(p,'double','flag_PD_end','0','Location of PD frames. To maintain back compatability');
...
try
    ...
    fprintf(fid,'%s',['mpi2d TEST(ver ' num2str(version) ') ran at ' num2str(date) ' with the following params(' num2str(varargin{:}) ') on seriesNum: ' num2str(seriesNum) ' and sliceNum:' num2str(sliceNum)]);
    fprintf(fid,'\n');
    fclose(fid);
catch ME %#ok<NASGU>
    disp('Could not write to the log file');
end

if ~exist('Output','dir')
  [SUCCESS,MESSAGE,MESSAGEID]= mkdir('Output'); %#ok<ASGLU,NASGU>  % it claims it fails, but seems to work!
  if(SUCCESS==0);
    error('Output directory could not be created. Please check your current working directory and ensure that you have permission to write to this directory');
  end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    dicompwd = pwd;
    candidateFolders = dir('3D*');
    if(strcmp(DoThisFolder,''))
        FoldersToDo = {};
        for i=1:length(candidateFolders)
            if(~candidateFolders(i).isdir), continue; end
            dicomFiles = dir([candidateFolders(i).name '/*.dcm']);
            if(length(dicomFiles) < 8)   %changed to 10 temp to get dry runs for precontrast frames DEV
                disp([candidateFolders(i).name 'has less than 10 dicoms.  Skipping']);
                continue;
            end
    ...
    while(MyVal >= 0)
        ...
        for i=indexOfFoldersToDo
            ...
            for j=1:length(dicoms)
                ...
                sin_a=header.FlipAngle;
                phase_encodes=header.NumberOfPhaseEncodingSteps;
            end
            [time IX] = sort(time);
            time = time-time(1);
            ...
            windowWidth = sx/2.5;
            windowHeight = sy/2.5;
            rangex = 1:(size(fullcinemri1,1)*2);
            rangey = 1:(size(fullcinemri1,2)*2);
            ...
            temp.sin_a=num2str(sin_a);
            temp.sin_asmall=num2str(4);
            temp.nor=num2str(phase_encodes);
            temp.slice_satfreq=num2str(5);
            temp.TD=num2str(20);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    shiftfilename2 = ['Output/shifts.study' num2str(seriesNum) '.slice' num2str(sliceNum) '.txt'];
    if(~exist(shiftfilename) && ~exist(shiftfilename2))
        disp('No shift file found');
        return;
    end
    shifts = load(shiftfilename2);
    %just do mpi2d.par
       ...
       % if only shiftsMAN exists, do as previous, just apply shifts,
        ...
        % make below a function, so can call it from other registration stages
            ...
            for t=1:size(shifts,1)
              temp = circshift(mycine(rangex,rangey,t),shifts(t,1:2));  %EVRD 3/30/12
              cinemri1(:,:,t) = temp;
            end
        end
    end
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end;if ran(stg,1.5,'Show movie of MRI data to select a Reference Frame and to determine cropping dimensions');
  ...
  for i=min(ranget):1:max(ranget)
  imagesc(fullcinemri1(rangex,rangey,i));
  colormap gray
  pause
  end
pause
...
if ran(stg,2,'REGISTRATION OF MRI DATA');
...
manualShiftsVector = auto_registration(cinemri,ranget,1);
...
%         mycine = mycine(:,:,IX);
    ...
    if ~(exist(['Output/shiftsMAN.study',num2str(seriesNum),'.slice',num2str(sliceNum),'.txt'])) && ~(exist(['Output/shifts.study',num2str(seriesNum),'.slice',num2str(sliceNum),'.txt']))
        dummy_timestamps=1;  % just needs to exist for the call, a hack. 
        shifts_out = NeighboringTemporalSmoothingRegistration(mycine, rangex, rangey, 10, dummy_timestamps); % doesn't use timestamps
...
end;if ran(stg,2.81,'Temporal smoothing an PD scaling');
disp('Retrieving images and upsampling...');
ranget1=1:ranget(end);
    dicoms = dir([infile '*.dcm']);
    legacy = 0;
    if(isempty(dicoms))
        %Now check if this is a legacy par file
        if(exist(infile))
            legacy = 1;
            %this is probably either a mat or a raw file
            cinemri1 = RetrieveMovieFromMat(infile,rangex, rangey,ranget1,flipy,flipx,rotNinety,inendi,intype,inpnxy);
            mycine = cinemri1;
        ...
        counter=1;
        if ~isempty(framesToSelect)
          for j=framesToSelect    % EVRD 8/17/11, to sele title('Reference Frame')ct ranget. 
            if (j <=max(ranget1)) && (j >=min(ranget1))
                temp = mpi_upsampleImages(dicomread([infile dicoms(j).name]));
                HeaderInfo = dicominfo([infile dicoms(j).name]);
                mytime(counter) = str2num(HeaderInfo.AcquisitionTime);
                mycine(:,:,counter) = temp;  % see note just below(rangex, rangey);
        ...
        [mytime, IX] = sort(mytime); % should we flag if any out of order??
        mycine = mycine(:,:,IX);
        pd_im=mycine(rangex,rangey,2)+mycine(rangex,rangey,2)+mycine(rangex,rangey,2)+mycine(rangex,rangey,2);
        mycine=mycine(:,:,ranget);
...
%         mycine = mycine(:,:,IX);
    ...
    timestamps = x.timeStamp;
    if(length(timestamps) ~= size(mycine,3))
        timestamps = timestamps(min(ranget1):end);
        if(length(timestamps) ~= size(mycine,3))
            timestamps = x.timeStamp(ranget1);
            if(length(timestamps) ~= size(mycine,3))
                disp('Houston, we have a timestamp misalignment problem - can skip, doesnt use in 2.8');
                %keyboard;
            end
    ...
    if ~(exist(['Output/shiftsMAN.study',num2str(seriesNum),'.slice',num2str(sliceNum),'.txt'])) && ~(exist(['Output/shifts.study',num2str(seriesNum),'.slice',num2str(sliceNum),'.txt']))
        dummy_timestamps=1;  % just needs to exist for the call, a hack. 
        shifts_out = NeighboringTemporalSmoothingRegistration(mycine, rangex, rangey, 10, dummy_timestamps); % doesn't use timestamps
        shifts=[shifts_out'; actualFrameNumbers(ranget)]';
    else
        
        % make below a function, so can call it from other registration stages
        [shifts, original_shifts firstShift]=handle_shifts(seriesNum, sliceNum, rangex, rangey, actualFrameNumbers(ranget));
    end
...
%         rangey = rangey;
    ...
    for t=1:length(shifts)
     ...
     pd_im=pd_im./4;
     smthr=fspecial('gaussian',size(pd_im),4.0);
     pd_blur=imfilter(pd_im,smthr);
     save(['Output/pd_blur.study' int2str(seriesNum) '.slice' int2str(sliceNum) '.mat'],'pd_blur');
     for cnt=1:1:size(cinemri1,3)
     cinemri2(:,:,cnt)=cinemri1(:,:,cnt)./pd_blur;       %for testing have to speed this step up    
     end
     clear cinemri1
     %cinemri1=(sum(sum(pd_im))/(size(pd_im,1)*size(pd_im,2))).*cinemri2;
     cinemri1=200.*cinemri2;
...
if ran(stg,9.2,'Gated Process with defo');
...
try
    ...
    fullcinemri1=cinemri1;
    pd_frames=fullcinemri1(:,:,1:pd_end);
catch
        dicoms = dir([infile '*.dcm']);
        if(isempty(dicoms))
            disp('No dicoms found.  Cannot do Temporal Smoothing registration');
        ...
        for t=1:size(dicoms,1)
            fullcinemri1(:,:,t) = mpi_upsampleImages(dicomread([infile dicoms(framesToSelect(t)).name]));
        end
        pd_frames=fullcinemri1(:,:,1:pd_end);
        
 end
    run_registration(fullcinemri1(:,:,pd_end+1:end),rangey,rangex,referenceFrame, numSkip,numPreContrast_bld,numPreContrast_tiss,seriesNum);
  temp=strcat('Output/',int2str(seriesNum),'/cinemri_curv_fit1.mat');
 load(temp);
 temp=strcat('Output/',int2str(seriesNum),'/buffer_cinemri.mat');
load(temp);
mask=ones(size(buffer_cinemri,1),size(buffer_cinemri,2));
cinemri2=ANTS_DMBR1(buffer_cinemri,cinemri_curv_fit,mask,0.1,0.5,0.5);
!rm -f *mhd
!rm -f *raw
!rm -f *gz
!rm -f *txt
% run_registration(cinemri2_tmp(:,:,1:end),rangey,rangex,referenceFrame, numSkip,numPreContrast_bld,numPreContrast_tiss,seriesNum+10);
% 
%  temp=strcat('Output/',int2str(seriesNum+10),'/cinemri_curv_fit1.mat');
%  load(temp);
%  temp=strcat('Output/',int2str(seriesNum),'/buffer_cinemri.mat');
% load(temp);
% cinemri2=ANTS_DMBR(buffer_cinemri(:,:,:),cinemri_curv_fit(:,:,:),0.1,0,0);
% !rm -f *mhd
% !rm -f *raw
% !rm -f *gz
% !rm -f *txt
       
cinemri1_nup=zeros(size(cinemri2,1),size(cinemri2,2),size(cinemri2,3)+pd_end);
cinemri1_nup(:,:,1:pd_end)=pd_frames;
cinemri1_nup(:,:,pd_end+1:end)=cinemri2;
cinemri1_nup(:,:,1:pd_end)=pd_frames;
clear cinemri1
...
save(['cinemri2.study' num2str(seriesNum) '.slice' num2str(sliceNum) '.mat'],'cinemri1');
temp=strcat(['Output/shiftsMAN.study' num2str(seriesNum) '.slice' num2str(sliceNum) '.txt']);
mat2txt(temp,shfts,framesToSelect,ranget);

end


if ran(stg,9.3,'Ungated Process with defo');
load('pd_end.mat');
 flag_PD_end=0;   
% try  %%%%%% added for processing ungated data with par files
%     load(['Output/cinemri1.study',num2str(seriesNum),'.slice',num2str(sliceNum),'.mat']);
% 
%     fullcinemri1=cinemri1;
%     pd_frames=fullcinemri1(:,:,1:pd_end);
...
% seriesnum_dry=64072;
% sliceNum_dry=4;
% load(['Output/cinemri1.study',int2str(seriesnum_dry),'.slice',int2str(sliceNum_dry),'.mat']);
% cine_dry=cinemri1;       
% clear cinemri1;
%  for j=1:1:size(fullcinemri1,3)+4
%         if j<=4
%     fullcinemri3(:,:,j)=cine_dry(:,:,j);
%         else
%     fullcinemri3(:,:,j)=fullcinemri1(:,:,j-4);
%         end
%  end
%  load(['timeStampSer',int2str(seriesNum),'.mat']);
%  p=polyfit(1:length(timeStamp),timeStamp(:)',4);
%  clear timeStamp;
%  timeStamp(:,1)=polyval(p,1:size(fullcinemri3,3));
%  timeStamp(find(timeStamp<0))=0;
%  save(['timeStampSer',int2str(seriesNum),'.mat'],'timeStamp');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 if flag_PD_end==0       
 pd_frames=fullcinemri1(:,:,1:pd_end);
 else
   pd_frames=fullcinemri1(:,:,end-9:end-1);
 end
%  fullcinemri2=ANTS_rigid(fullcinemri1,pd_frames(:,:,3));
%   !rm -f *mhd
%  !rm -f *raw
%  !rm -f *nii
%  !rm -f *txt
%[shifts_out, fullcinemri2] = NeighboringTemporalSmoothingRegistration(fullcinemri1, x-40:x+40, y-40:y+40, 15, 15);

 prebolus_flag=0;
 save_flag=1;
 
 processUngated(fullcinemri1(:,:,1:end),pd_frames,seriesNum,sliceNum,prebolus_flag,save_flag,rangex,rangey,framesToSelect,ranget,flag_PD_end,pd_end);
       
end

if ran(stg,9.4,'Ungated throw frames');
dicoms = dir([infile '*.dcm']);
        if(isempty(dicoms))
            disp('No dicoms found.  Cannot do Temporal Smoothing registration');
            keyboard;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 pd_frames=fullcinemri1(:,:,1:5);
%  fullcinemri2=ANTS_rigid(fullcinemri1,pd_frames(:,:,3));
%   !rm -f *mhd
%  !rm -f *raw
%  !rm -f *nii
%  !rm -f *txt
[RV LV]=FindLVRV(fullcinemri1);
...
%[shifts_out, fullcinemri2] = NeighboringTemporalSmoothingRegistration(fullcinemri1, x-40:x+40, y-40:y+40, 15, 15);
 ...
 prebolus_flag=0;
 save_flag=1;
 processUngated1(fullcinemri1,pd_frames,seriesNum,sliceNum,prebolus_flag,save_flag,rangex,rangey,framesToSelect,ranget);
end


if ran(stg,2.11,'zero-Shift registration');
...
%  a=deltaSIcurves(1,end-15:end-10)./gdcurves(1,end-15:end-10);
%  b=sum(a)/6;
%  gdcurves=b.*gdcurves;
%  figure;
%   plot(deltaSIcurves(1,:),'r');
%   hold on
%   plot(gdcurves(1,:));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [sx sy st] = size(cinemri1);
     ...
     %%%%%%%%%%%%%%%%%%%%%%%%%%%Stacom%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     flag_stacom=1;
%    % clear endo epi angle
%    if flag_stacom==1
%     disp('Please select the correct myocardial mask file')
%    [Mask_name,Mask_path] = uigetfile('*.png','Select the correct mask file');
%    
%    mask=imread([Mask_path,'/',Mask_name]);
%     mask=mask';
%    B = bwboundaries(mask,8,'holes');
%    area(1) = polyarea(B{1}(:,1),B{1}(:,2));
%    area(2) = polyarea(B{2}(:,1),B{2}(:,2));
%     
%     if( area(1) < area(2) )
%         endo_pts = B{1};
%         epi_pts = B{2};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end;if ran(stg,3.14,'Mid wall');
 infilename=strcat('Output/','cinemri1.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.mat');
    load(infilename);
mpi_computeMidwall(cinemri1,sliceNum, seriesNum, outpath,referenceFrame) 
...
%     epi = load([outpath 'epi_polyCoords.study' int2str(seriesNum) '.slice' int2str(sliceNum)]);
     start_angle = load([outpath 'Roi_start_angle.study' int2str(seriesNum) '.slice' int2str(sliceNum)]);
%     figure(seriesNum*10 + 2*sliceNum+1);

     curves =ExtractCurves(cinemri1,sliceNum,seriesNum,outpath,start_angle,numAzimuthalRegions,referenceFrame);
      end
%     curves1=curves;
%     clear curves
%     for i=1:1:size(curves1,1)
%     curves(i,:)=(curves1(i,:)./(mean(curves1(i,1:4)))).*mean(mean(curves1(:,1:4)));  %To correct for coil sensitivity PD SCALING   DEV
%     end
   ...
   if flagPixelwise
    curvefilename=strcat(outpath,'curves_pixelwise.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.mat');    %To treat pixelwise differently
...
%     curves=mpi_applyRegions(cinemri1,sliceNum,seriesNum,outpath, referenceFrame, numAzimuthalRegions, flagPixelwise, numRadialRegions, pixelSizeRatio);
    deltaSIcurves=mpi_si2deltaSI(sliceNum,seriesNum,outpath,numPreContrast_bld, numPreContrast_tiss, numSkip, 0,ranget);
    
    LV = load([outpath 'blood_polyCoords.study' int2str(seriesNum) '.slice' int2str(sliceNum)]);   
    epi = load([outpath 'epi_polyCoords.study' int2str(seriesNum) '.slice' int2str(sliceNum)]); 
    endo = load([outpath 'endo_polyCoords.study' int2str(seriesNum) '.slice' int2str(sliceNum)]);
    load(['timeStampSer' num2str(seriesNum) '.mat']);
    close allsin_a
...
% Read in contours and display - still working on improvementssin_a
  infilename=strcat(outpath,'cinemri1.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.mat');
  load(infilename);
  mpi_showRegions_old(cinemri1,sliceNum, seriesNum, outpath, numAzimuthalRegions, numRadialRegions, 25);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end;if ran(stg,3.4,'Convert the SI curves to [Gd] or deltaSI ');
...
%%%%%%%%%%%%%DEV TEMP ADDTN%%%%%%%%%%%%%%%%%%
b=round(seriesNum/100);
b1=b/38;
seriesNum1=seriesNum+10;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% if sliceNum>1  
%  if seriesNum==100
%      T2star_corr_flag=1;
...
if T2star_corr_flag==1;  %%%%%  ( T2 star correction using two echos)
    TE1=data_info.EchoTimeAIF;
    TE2=data_info.EchoSpacingAIF+TE1;
    sin_PD=sin(data_info.ProtonDensityFlipAngle/180*pi);
    sin_alpha=sin(data_info.FlipAngle/180*pi);
    
    Echo1=curves(1,pd_end+1:end);
     curvefilename=strcat(outpath,'curves.study',int2str(seriesNum+1),'.slice',int2str(sliceNum),'.mat');
    load(curvefilename);
     Echo2=curves(1,pd_end+1:end);
    
    
    for i=1:length(Echo1)
        
        T2_star(i)=(TE2-TE1)/log(Echo1(i)/Echo2(i));
        
    end
    
    scale_t2star1=exp(-TE1./T2_star);
    AIF_new1=Echo1./scale_t2star1;
% end


        curves(1,pd_end+1:end)=(AIF_new1);  
end

if flag_PD_end==0;
    Mz0=curves(:,1:pd_end);
else
    Mz0=curves(:,end-pd_end+1:end);
end

if flagPixelwise
    curvefilename=strcat(outpath,'curves_pixelwise.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.mat');
    load(curvefilename);
else
    curvefilename=strcat(outpath,'curves.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.mat');
...
if  useDeltaSI~=1    %  12/30/03.
    ...
    slice_sat_no=[1 1 1 1 1 1 1 1 1];
    
    %          gdcurves=mpi_si2gdconc(Mz0,curves,slice_sat_no(sliceNum),sin_a,sin_asmall,TD,nor,flag_PD_end,pd_end); % hard coded 2 for ISMRM ab
%            gdcurves=mpi_si2gdconc_test2(Mz0,curves(:,pd_end+1:end),slice_sat_no(sliceNum),nor,pd_end);
%           [gdcurves T1 T_1]=conc_gd1(curves,Mz0,sliceNum,seriesNum,pd_end);  %%%%%% New mpi2d stg=3.4 added by Apoorva on 5/15/17
            [gdcurves T1]=conc_gd_v2(curves,seriesNum,data_info);
%               [gdcurves T1]=conc_gd_v2_1(curves,seriesNum);
%               [gdcurves T1]=conc_gd_v3(curves,seriesNum,data_info);
    % Old conversion method 
%                  [gdcurves t1_value]=mpi_si2gdconc_hybrid1(Mz0,curves(:,pd_end+1+numSkip:end),slice_sat_no(sliceNum),sliceNum,seriesNum); 
    ...
    figure(seriesNum*10 + 2*sliceNum+1), clf, hold on, plot(gdcurves(1,:),'k');
    for region=1:numAzimuthalRegions
        plot(gdcurves(region+1,:)','Color',mycolors(region,:));
        hold on;
    ...
    h = figure;
    numPreContrast_tiss=2;
    NumPreContrast_bld=2;
    numSkip=2
    deltaSIcurves=mpi_si2deltaSI(Mz0,curves,sliceNum,seriesNum,outpath,numPreContrast_bld, numPreContrast_tiss, numSkip, scaleToStudyOne,ranget,flag_PD_end,pd_end);

%%%%%%%%%%%%%%%%%%%%%%%%%TEST%%%%%%%%%%%%%%%%%%%%  
% load AIF.mat
% 
% deltaSIcurves(1,1:size(AIF,2))=AIF(3,:);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     g=fspecial('gaussian',[1 3],1);
%     for i=1:1:size(deltaSIcurves,1)
%     deltaSIcurves(i,:)=imfilter(deltaSIcurves(i,:),g);
%     end
...
if flag_PD_end==0
     framesToSelect=framesToSelect(1:end);
     framesToSelect_AIF=framesToSelect_AIF(1:end);
else
    framesToSelect=framesToSelect(1:end-pd_end);
    framesToSelect_AIF=framesToSelect_AIF(1:end-pd_end);
end
...
if useDeltaSI~=1
   ...
   gdcurves=mpi_useAlternateAIF_gdConcVersion(seriesNumAIF, sliceNumAIF, scaleAIF, sliceNum,seriesNum, outpath, numPreContrast_bld, numPreContrast_tiss, numSkip,framesToSelect, ranget, timeStampFile, timeStampFileAIF,framesToSelect_AIF,ranget_AIF,delta_t);
   close(h);
   
   % gdcurves=gdcurves(:,1:end-10);
   showcurves(gdcurves,'[Gd]')
   drawnow
   outfilename=strcat(outpath,'gdcurves.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.AIF_',int2str(seriesNumAIF),'_',int2str(sliceNumAIF),'_',num2str(scaleAIF),'.mat');
   save(outfilename, 'gdcurves')
   
 
   if exist(timeStampFile)
        fit_model_2comp_new(seriesNumAIF, sliceNumAIF, scaleAIF, sliceNum,seriesNum,outpath,'gdcurves',delta_t,numAzimuthalRegions, numRadialRegions, flagPixelwise, 0, 0, fixedVp, fixedDelay, modelType)
   else
    ...
    [deltaSIcurves,scaleAIF]=mpi_useAlternateAIF(seriesNumAIF, sliceNumAIF,scaleAIF, sliceNum, seriesNum,outpath,numPreContrast_bld, numPreContrast_tiss, numSkip,framesToSelect, ranget,timeStampFile, timeStampFileAIF,delta_t);%%% using tail frames to scale here...NP 082609
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  clusterFlag=0; modelType='full' %#ok<NOPRT>
     ...
     curvefilename=strcat(outpath,'gdcurves.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.AIF_',int2str(seriesNumAIF),'_',int2str(sliceNumAIF),'_',num2str(scaleAIF),'.mat');
     load(curvefilename);
     fixedVp=99;
     ...
     if(flagPixelwise);
        infilename=strcat(outpath,'cinemri1.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.mat');
        load(infilename);
        mpi_displayPixelwise(cinemri1,sliceNum,seriesNum,outpath,gdcurves, referenceFrame, numPreContrast_bld, numPreContrast_tiss, useIntegralLinearFit,fixedDelay, fixedVp, seriesNumAIF, sliceNumAIF, scaleAIF,modelType)
...
%      else
        ...
        mpi_displayRegionValues(cinemri1,sliceNum,seriesNum,outpath,numAzimuthalRegions, numRadialRegions, referenceFrame,  clusterFlag, flagPixelwise, useIntegralLinearFit, fixedDelay, fixedVp, seriesNumAIF, sliceNumAIF, scaleAIF, modelType, flagTimeStamps,fil(I))
     end
     end
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if flagPixelwise==0
      showfits(gdcurves,seriesNum,sliceNum,outpath,numAzimuthalRegions, numRadialRegions, 0,flagPixelwise, useIntegralLinearFit, fixedDelay, fixedVp, seriesNumAIF, sliceNumAIF, scaleAIF, modelType, lastFrame, timeStampFile,timeStampFileAIF,fil(I));
    end
    I=I+1;
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for curve=2:size(deltaSIcurves,1)
        for t=1:(st-slidingWindowWidth)
            s = robustfit(((0:slidingWindowWidth)),deltaSIcurves(curve,t:(t+slidingWindowWi0dth)));
...
if flag_PD_end==0
    ...
    framesToSelect=framesToSelect(1:end-15);
    framesToSelect_AIF=framesToSelect_AIF(1:end-15);
end
...
if useDeltaSI~=1
   ...
   gdcurves=mpi_useAlternateAIF_gdConcVersion(seriesNumAIF, sliceNumAIF, scaleAIF, sliceNum,seriesNum, outpath, numPreContrast_bld, numPreContrast_tiss, numSkip,framesToSelect, ranget, timeStampFile, timeStampFileAIF,framesToSelect_AIF,ranget_AIF,delta_t);
  try
      load(['Output/firstpass_cutoff_',int2str(seriesNum),'_',int2str(sliceNum),'.mat']);
  catch
   firstpass_cutoff=fermi_auto_firstpass(gdcurves(1,:),seriesNum,sliceNum);
  end
   gdcurves=gdcurves(:,1:firstpass_cutoff);
   %fixedDelay=99;
   outfilename=strcat(outpath,'gdcurves_fermi.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.AIF_',int2str(seriesNumAIF),'_',int2str(sliceNumAIF),'_',num2str(scaleAIF),'.mat');
   save(outfilename, 'gdcurves')
   
   if exist(timeStampFile)
        fit_modelFermi(seriesNumAIF, sliceNumAIF, scaleAIF, sliceNum,seriesNum,outpath,'gdcurves_fermi',delta_t,numAzimuthalRegions, numRadialRegions, flagPixelwise, 0, 0, fixedVp, fixedDelay, modelType, timeStampFile, timeStampFileAIF);
   else
      %  fit_model_2comp(seriesNumAIF, sliceNumAIF, scaleAIF, sliceNum,seriesNum,outpath,'deltaSIcurves',delta_t,numAzimuthalRegions, numRadialRegions, flagPixelwise, 0, 0, fixedVp, fixedDelay, modelType, framesToSelect,ranget);
   end
else
   ...
   firstpass_cutoff=fermi_auto_firstpass(deltaSIcurves(1,:),seriesNum,sliceNum);
     end
  
   deltaSIcurves=deltaSIcurves(:,1:firstpass_cutoff);
     outfilename=strcat(outpath,'deltaSIcurves_fermi.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.AIF_',int2str(seriesNumAIF),'_',int2str(sliceNumAIF),'_',num2str(scaleAIF),'.mat');
   save(outfilename, 'deltaSIcurves') 
   ...
   if exist(timeStampFile)
        fit_modelFermi(seriesNumAIF, sliceNumAIF, scaleAIF, sliceNum,seriesNum,outpath,'deltaSIcurves_fermi',delta_t,numAzimuthalRegions, numRadialRegions, flagPixelwise, 0, 0, fixedVp, fixedDelay, modelType, timeStampFile, timeStampFileAIF);
   else
      %  fit_model_2comp(seriesNumAIF, sliceNumAIF, scaleAIF, sliceNum,seriesNum,outpath,'deltaSIcurves',delta_t,numAzimuthalRegions, numRadialRegions, flagPixelwise, 0, 0, fixedVp, fixedDelay, modelType, framesToSelect,ranget);
   end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %   mpi_displayRegionValues(cinemri1,sliceNum,seriesNum,outpath,numAzimuthalRegions, numRadialRegions, referenceFrame,  clusterFlag, flagPixelwise, useIntegralLinearFit, fixedDelay, fixedVp, seriesNumAIF, sliceNumAIF, scaleAIF, modelType)
  %   curvefilename=strcat(outpath,'deltaSIcurves.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.AIF_',int2str(seriesNumAIF),'_',int2str(sliceNumAIF),'_',num2str(scaleAIF),'.mat')
     curvefilename=strcat(outpath,'gdcurves_fermi.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.AIF_',int2str(seriesNumAIF),'_',int2str(sliceNumAIF),'_',num2str(scaleAIF),'.mat');
     load(curvefilename);
     showfits(gdcurves,seriesNum,sliceNum,outpath,numAzimuthalRegions, numRadialRegions, 0,flagPixelwise, useIntegralLinearFit, fixedDelay, fixedVp, seriesNumAIF, sliceNumAIF, scaleAIF, modelType, lastFrame, timeStampFile,timeStampFileAIF);
...
for i=1:1:size(cinemri1,3)
cinemri1_tmp(:,:,i)=interp2(cinemri1(:,:,i),upsampleFactor-1,'cubic');
end
   clear cinemri1
   cinemri1=cinemri1_tmp;
    [sx,sy,~] = size(cinemri1);
...
%         myscaledktrans = myscaledktrans / max(myscaledktrans);
        myscaledktrans = myscaledktrans / Scal_max;
        ...
        epi = load([outpath 'epi_polyCoords.study' int2str(seriesNum) '.slice' int2str(sliceNum)]);
        angle = load([outpath 'Roi_start_angle.study' int2str(seriesNum) '.slice' int2str(sliceNum)]);
%          mid = load([outpath 'midwall_polyCoords.study' int2str(seriesNum) '.slice' int2str(sliceNum)]);
        legacy = 0;
...
%         end
        ...
        if(~legacy)
            labeled = mod(labeled,6);
        end
        for i=1:length(r)
            myregionalMask(r(i)+5,c(i)+5) = labeled(i)+1;
        end
...
%                        myrgb(x,y,:) = radialWeight(x,y)*c1 + (1-radialWeight(x,y))*c2;
        ...
        set(cb,'YTickLabel',{round(10*(0:Scal_max/10:Scal_max))/10});
...
%          extendedMid = vertcat(mid((end-9):end,:),mid,mid(1:10,:));
        clear smoothedExtendedEndo smoothedExtendedEpi smoothedExtendedMid
...
%          smoothedExtendedMid(:,1) = conv(fspecial('gaussian',[length(extendedMid) 1],.5),extendedMid(:,1),'same');
%         smoothedExtendedMid(:,2) = conv(fspecial('gaussian',[length(extendedMid) 1],.5),extendedMid(:,2),'same');
        
        
        smoothedEndo = smoothedExtendedEndo(11:(end-10),:); %#ok<NASGU>
        smoothedEpi = smoothedExtendedEpi(11:(end-10),:); %#ok<NASGU>
%         smoothedMid = smoothedExtendedMid(11:(end-10),:); %#ok<NASGU>
%         plot(smoothedEndo(:,1), smoothedEndo(:,2));
%         plot(smoothedEpi(:,1), smoothedEpi(:,2));
%         plot(smoothedMid(:,1), smoothedMid(:,2),'r');
        title('Regional K-trans');
        drawnow
        
        saveas(seriesNum,['slice',num2str(sliceNum),'_dia.jpg'])
    end
end
...
%
function par=buf2par(par,buf,test1)
...
%           a=mpi_upsampleImages(imgrr(rangey,rangex,ranget));
          a=mpi_upsampleImages(recon_data_inter(rangey,rangex,ranget));
