...
sat_bldcurve=bldcurve;
   ...
   tmp=0:nTimes-1;
      ttimes = delta_t * tmp';
      %ttimes = tmp';
 outfilename=strcat(outpath,'ttimes',int2str(studyNum),'.slice',int2str(sliceNum),'.AIF_',int2str(seriesNumAIF),'_',int2str(sliceNumAIF),'_',num2str(scaleAIF),'.mat');
   save(outfilename, 'ttimes')
...
%      [est_curves(:,i), tmpbldcurve]=mpi_sat_fwdModel(bldcurve, sat_bldcurve, delta_t, kwi(i), kwo(i), t_delay(i), est_fb(i), spillover(i));
   ...
   est_curves(:,i)=est_curves((jitter+1):newRate:nTimes,i);   % downsampling ste
      ...
      set(gca,'FontSize',16); set(gcf,'Color',[1 1 1]);
       outfilename=strcat(outpath,'tisscurve.study',int2str(studyNum),'.slice',int2str(sliceNum),'.AIF_',int2str(seriesNumAIF),'_',int2str(sliceNumAIF),'_',num2str(scaleAIF),'.mat');
   save(outfilename, 'tisscurve');
    outfilename=strcat(outpath,'est_curves.study',int2str(studyNum),'.slice',int2str(sliceNum),'.AIF_',int2str(seriesNumAIF),'_',int2str(sliceNumAIF),'_',num2str(scaleAIF),'.mat');
   save(outfilename, 'est_curves')
   save('bldcurve.mat','bldcurve')
%keyboard
...
%      end
       ...
       fignum=gcf;
       firstTime=1;
    h=figure(77); hold on; set(gcf,'position',[100 100 800 400])
...
% xlabel('Time')
% ylabel('[Gd]')
% print(h,'-djpeg',[outpath,'figs/fits.study',int2str(studyNum),'.slice',int2str(sliceNum),'.jpg']);
% surf(est_curves,'EdgeColor','none','FaceColor','interp')
% ylabel('Time')
% zlabel('[Gd]')
% xlabel('Regions')
...
% CaptureFigVid([-10,10;-40,10;-70,10;-100,10;-130,10;-160,10;-190,10;-210,10;-240,10;-270,10;-300,10;-330,10;-360,10;-390,10], 'fits',OptionZ)
% 
% 
...
% 
   % only write out for simulating  (series  99x are the simulations)
      ...
      estcurves=[est_curves'];
      save(outfilename, 'estcurves');
