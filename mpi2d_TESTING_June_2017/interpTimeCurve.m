function  [newTimeCurve,inter_time]=interpTimeCurve(selectedTimeStamps, delta_t, origTimeCurve)
% newTimeCurve will have length totaltime/delta_t
% example call: 
%   bldcurve=interpTimeCurves(timeStampFileAIF, delta_t, bldcurve);

showfigs=0;
if length(origTimeCurve) >length(selectedTimeStamps)
origTimeCurve=origTimeCurve(1:length(selectedTimeStamps));
end
...
%    end
   timeStamps=squeeze(selectedTimeStamps(:,1)-selectedTimeStamps(1,1));   % slice 1 assumedly
    %timeStamps=squeeze(selectedTimeStamps(:,1));
   %keyboard
...
%   extrapval=mean(bldcurve(nTimes-4:nTimes-1));
   extrapval=0;
inter_time=delta_t.*(0:nTimes-1);
   newTimeCurve = interp1(origSampleTimes, origTimeCurve,delta_t.*(0:nTimes-1),'cubic',extrapval);
  %newTimeCurve = interp1(origSampleTimes, origTimeCurve,0:delta_t:end_t,'cubic',extrapval);
