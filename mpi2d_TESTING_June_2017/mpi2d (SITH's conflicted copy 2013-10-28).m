...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if(strcmp(DoThisFolder,''))
        ...
        for i=1:length(candidateFolders)
            ...
            if(length(dicomFiles) < 10)   %changed to 10 temp to get dry runs for precontrast frames DEV
    ...
    while(MyVal >= 0)
        ...
        for i=indexOfFoldersToDo
            ...
            clear temp;  
            temp.rangex = [num2str(min(rangex)-20) ':' num2str(max(rangex)+20)];  % Changed Dev for testing 6/13
            temp.rangey = [num2str(min(rangey)-20) ':' num2str(max(rangey)+20)];
            temp.infile = [infile '/'];
...
manualShiftsVector = auto_registration_trail1(cinemri,ranget,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
end;if ran(stg,2.92,'MB registration');  %DEV
    disp('Retrieving images...');
  ...
  counter=1;
   ...
   cinemri1=cinemri1(:,:,framesToSelect);     
   run_registration(cinemri1(:,:,:),rangey,rangex,referenceFrame, numSkip,numPreContrast_bld,numPreContrast_tiss,seriesNum);
...
counter=1;
  ...
  if(~isempty(strfind(infile,'.mat')))
        ...
        for t=1:length(actualFrameNumbers)
            fullcinemri1(:,:,t) = mpi_upsampleImages(dicomread([infile dicoms(actualFrameNumbers(t)).name]));
            HeaderInfo = dicominfo([infile dicoms(t).name]);
            mytime(t) = str2num(HeaderInfo.AcquisitionTime);
        end
        [mytime, IX] = sort(mytime); %#ok<ASGLU>
...
load(temp);
cinemri=fullcinemri1(rangex,rangey,:);

cinemri1=ANTS_DMBR(cinemri(:,:,:),cinemri_curv_fit(:,:,:));


load(['Output/',num2str(seriesNum),'/pd_frames.mat'])
 
cinemri1=zeros(size(cinemri2,1),size(cinemri2,2),size(cinemri2,3)+4);

cinemri1(:,:,1:4)=pd_frames(:,:,:);
cinemri1(:,:,5:end)=cinemri2;

save(['Output/',num2str(seriesNum),'/cinemri1.mat'],'cinemri1');
cinemri1=cinemri1(rangex,rangey,:);
...
if ran(stg,9.1,'Test');
 counter=1;
        ...
        [mytime, IX] = sort(mytime); %#ok<ASGLU>
        fullcinemri1 = fullcinemri1(rangex,rangey,IX);
%%%%%%%%%%%%%%%%%%%%%%%ADD DRY RUN%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
seriesnum_dry=64000;
load(['Output/cinemri1.study',int2str(seriesnum_dry+(sliceNum-1)*1000),'.slice',int2str(sliceNum),'.mat']);
cine_dry=cinemri1;       
clear cinemri1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
cinemri2=fullcinemri3(:,:,:);
a=zeros(size(cinemri2,1),size(cinemri2,2),1);
for i=min(ranget):1:size(cinemri2,3)
    a=a+cinemri2(:,:,i);
end
a=a./size(cinemri2,3);
%a=cinemri2(:,:,2);
cinemri3=ANTS_rigid(cinemri2(:,:,:),a);
!rm -f *mhd
!rm -f *raw
!rm -f *nii
!rm -f *txt
pd_frames=cinemri3(:,:,1:4);
cinemri1=cinemri3(:,:,:);
save(['Output/cinemri1.study' num2str(seriesNum) '.slice' num2str(sliceNum) '.mat'],'cinemri1');
save(['Output/pd_frames.study' num2str(seriesNum) '.slice' num2str(sliceNum) '.mat'],'pd_frames');
% load(['Output/cinemri1.study' num2str(seriesNum) '.slice' num2str(sliceNum) '.mat']);
% load(['Output/pd_frames.study' num2str(seriesNum) '.slice' num2str(sliceNum) '.mat']);
%  prebolus_flag=0;
%  save_flag=1;
%  processUngated(cinemri1(:,:,:),pd_frames,seriesNum,sliceNum,prebolus_flag,save_flag);

end

if ran(stg,9.2,'Test2');
 disp('Retrieving images...');
  load(['Output/cinemri1.study' num2str(seriesNum) '.slice' num2str(sliceNum) '.mat']);
  run_registration(cinemri1(:,:,5:end),rangey,rangex,referenceFrame, numSkip,numPreContrast_bld,numPreContrast_tiss,seriesNum);
clear cinemri1
temp=strcat('Output/',int2str(seriesNum),'/cinemri_curv_fit1.mat');
load(temp);
temp=strcat('Output/',int2str(seriesNum),'/cinemri1.mat');
load(temp);
cinemri2=ANTS_DMBR(cinemri1(:,:,:),cinemri_curv_fit(:,:,:));
!rm -f *mhd
!rm -f *raw
!rm -f *nii
!rm -f *txt
...
% convert curves from si to gd conc.
  ...
  if  useDeltaSI~=1    %  12/30/03.
      slice_sat_no=[1:slice_satfreq,1:slice_satfreq];
     gdcurves=mpi_si2gdconc(curves,slice_sat_no(sliceNum),sin_a,sin_asmall,TD,nor); 
    ...
    h = figure;
    deltaSIcurves=mpi_si2deltaSI(curves,sliceNum,seriesNum,outpath,numPreContrast_bld, numPreContrast_tiss, numSkip, scaleToStudyOne,ranget);
%     
...
if useDeltaSI~=1
   ...
   gdcurves=mpi_useAlternateAIF_gdConcVersion(seriesNumAIF, sliceNumAIF, scaleAIF, sliceNum,seriesNum, outpath, numPreContrast_bld, numPreContrast_tiss, numSkip,framesToSelect, ranget, timeStampFile, timeStampFileAIF)
   close(h);
   drawnow
   ...
   if exist(timeStampFile)
    ...
    [deltaSIcurves,scaleAIF]=mpi_useAlternateAIF(seriesNumAIF, sliceNumAIF,scaleAIF, sliceNum, seriesNum,outpath,numPreContrast_bld, numPreContrast_tiss, numSkip,framesToSelect, ranget,timeStampFile, timeStampFileAIF);%%% using tail frames to scale here...NP 082609
...
% custom function by bmartin to find the fermi cut off location 3/13/13
fermiFrameTo = findLocalMinFermi(deltaSIcurves(1,:))
if (fermiFrameTo == false)
    
%  take the negative of the AIF so that the mins become the maxes and find
% the peaks of the new max's (which are really the mins when you multiply
% by -1 again)
% value = the values at the minimum points
% index = the index at which the minimums are located
[value, index] = findpeaks(-deltaSIcurves(1,:),'minpeakdistance',25);
slice = 1:length(deltaSIcurves(1,:));
figure(684094);

% show the AIF with the mins as red x's
plot(deltaSIcurves(1,:));
hold on;
plot(slice(index),-value,'xr');
hold off;

% allow the user to pick which min they want to get the correct fermi cut
% off point.  Ex: If you want the second red x, enter 2.
disp('Look at the red x''s and pick which x you''d like');
fermiFrameTo_index = input('Please enter integer number of which x you would like?  ');

fermiFrameTo = index(fermiFrameTo_index)
%close the cut off plot
close;
    
end

% save with new blood
   deltaSIcurves=deltaSIcurves(:,1:fermiFrameTo);  % added fermiFrameTo (replaced lastFrame - #) to only get first pass 
                                                   % bmartin 3/13/13
 figure(seriesNum*10 + 2*sliceNum+1),clf, hold on, plot(deltaSIcurves(1,:),'k');
   for region=1:numAzimuthalRegions
       plot(deltaSIcurves(region+1,:)','Color',mycolors(region,:));
   ...
   save(outfilename, 'deltaSIcurves') 
   %% run stage 5.2 right away:  %bmartin changed from 4.2 to 5.2 --- 3/11/13
% re  curvefilename=strcat(outpath,'deltaSIcurves.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.AIF_',int2str(seriesNumAIF),'_',int2str(sliceNumAIF),'_',num2str(scaleAIF),'.mat')




curvefilename=strcat(outpath,'deltaSIcurves_fermi.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.AIF_',int2str(seriesNumAIF),'_',int2str(sliceNumAIF),'_',num2str(scaleAIF),'.mat');
   load(curvefilename);
