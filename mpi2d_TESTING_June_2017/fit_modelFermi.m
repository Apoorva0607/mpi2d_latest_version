...
% bmartin 3/15/13
    ...
    %curvefilename=strcat(outpath,filenamePart1,'.study',int2str(studyNum),'.slice',int2str(sliceNum),'.AIF_',int2str(seriesNumAIF),'_',int2str(sliceNumAIF),'_',num2str(scaleAIF),'.mat');
   load(curvefilename);
   curves = eval(filenamePart1(1:end-6));   % may want to re-do this so not so tricky
...
%curvefilename=strcat(outpath,filenamePart1,'.study',int2str(studyNum),'.slice',int2str(sliceNum),'.AIF_',int2str(seriesNumAIF),'_',int2str(sliceNumAIF),'_',num2str(scaleAIF),'.mat');
   load(curvefilename);
   curves = eval(filenamePart1(1:end-6));
% %    %%%%% below was only used to create a mean single tis curve for display in model comparison paper   
% % tmp=mean(curves(2:end,:));curves=[curves(1,:);tmp];

   sat_bldcurve=curves(size(curves,1),:)';
...
% % resample to uniform time points:
 ...
 if exist('timeStampFile')
   flagTimeStamps=1;
%    
%    %bmartin 3/4/13 -- added the load(timeStampFileAIF) and changed the
%    %variable in a few places (will mark below) from timeStampFileAIF to
%    %timeStamp -- this was to allow interpTimeCurve.m to use the values in
%    %the timeStampFileAIF variable instead of using the filename as the
%    %variable
%    load(timeStampFileAIF);
%    
%    % bmartin 3/4/13 -- changed variable name timeStampFileAIF to timeStamp
%    bldcurve=interpTimeCurve(timeStamp, delta_t, bldcurve);
%    nTimes=length(bldcurve);
% 
...
%    clear tisscurve
%    for ii=1:nRegs
%        % bmartin 3/4/13 -- changed timeStampFileAIF to timeStamp 
%        tisscurve(:,ii)=interpTimeCurve(timeStamp, delta_t, origtisscurve(:,ii));
%    end
%  %  pause  % to see last region of tisscurve
%     load(timeStampFile);
%    sat_bldcurve=interpTimeCurve(timeStamp, delta_t, sat_bldcurve);
% 
...
for jj=1:nFitRuns  
   ...
   X0(3)=10.0; LB(3)=0.5; UB(3)=15.0  ;  % changed T0 term when the data is trucated at first-pass 08/22/08 NP 
   if jj==2
      ...
      X0(3)=10.0; LB(3)=0.0; UB(3)=15.0  ;  % changed to a much larger T0 term when the data is trucated at first-pass 08/22/08 NP 
  end
   if jj==3
      X0(2)=8.0; LB(2)=0.1*X0(2); UB(2)=2*X0(2); 
      X0(2)=8.0; LB(2)=0.01*X0(2); UB(2)=5*X0(2); % changed kwo value when the data is trucated at first-pass 08/22/08 NP
      X0(3)=4.0; LB(3)=0.5; UB(3)=25.0  ;  % T0 term 
      X0(3)=10.0; LB(3)=0.05; UB(3)=15.0  ;  % changed T0 term when the data is trucated at first-pass 08/22/08 NP 
   end
   if jj==4
      ...
      X0(3)=10.0; LB(3)=0; UB(3)=15.0  ;  % changed to a much larger T0 term when the data is trucated at first-pass 08/22/08 NP 
   end
...
if(jj==1)
    ...
    X0tmp(4)=-0.4; LBtmp(4)=-15.0; UBtmp(4)=0.8;  %0.8;
    if fixedVp==99
