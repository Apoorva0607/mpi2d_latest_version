...
clc
close all
  infile='/v/raid1a/gadluru/MRIdata/Cardiac/Prisma/P010917/ReconData/mat_files/3D/perfusion/aif/Pre_Interp_MID_172_slice_1_echo_1_MC.mat';
load(infile);
...
for i=indexOfFoldersToDo
    data.value = 0;
    if indexOfFoldersToDo==1
        
    folder=pwd;
    if(~exist('series100.slice1.par'))    %%% For dual echo AIF where series 100 is 1st echo and 101 would be 2nd echo
        sliceNum=1;
        seriesNum=100*i+addon;
        header.SeriesDescription=100;
    else
        sliceNum=1;
        seriesNum=101*i+addon;
        header.SeriesDescription=101;
    end
    else
    ...
    if ndims(image_data)==4
        ...
        cinemri1_tmp=(image_data(:,:,:));
        %               imagesc(cinemri1_tmp);
        %               mask=roipoly;
    end
    
    for count=1:st
        cinemri1_tmp1(:,:,count)=((rot90(fliplr((cinemri1_tmp(:,:,count))))));
