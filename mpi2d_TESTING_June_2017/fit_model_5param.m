function fitparams = fit_model_5param(seriesNumAIF, sliceNumAIF, scaleAIF, sliceNum,studyNum,outpath,filenamePart1,delta_t, numAzimuthalRegions, numRadialRegions, flagPixelwise, numSkip, curvesFromClusters, fixedVp, fixedDelay, modelType)
warning('off', 'all')
fitparams=1;
if nargin==0,
...
for ii=1:nRegs
   ...
   [x,fval,exitflag,output,lambda,grad,hessian]=fmincon('sat_func_fit_5param',X0,A,B,A,B,LB,UB,nonlcon,options,bldcurve(numSkip+1:nTimes),sat_bldcurve(numSkip+1:nTimes),tisscurve(:,ii),delta_t, 1, fixedDelay(ii), fixedVp, estGlobalDelay(ii));  
