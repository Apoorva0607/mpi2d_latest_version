function [X,Y,xCenter, yCenter, start_angle,bw1,bw2,bw3,bw5]= mpi_loadContours(img,sliceNum, studyNum, outpath, endoFlag)  
...
try
	...
	disp('Dropped rest or stress part of filename March 2002. Will now attempt to read old filename')	
	tmp=load(strcat(outp22ath,'epi_polyCoords.rest.study',int2str(studyNum),'.slice',int2str(sliceNum)));
end
