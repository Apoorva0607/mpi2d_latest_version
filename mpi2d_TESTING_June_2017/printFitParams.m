...
if(flagPixelwise);
    for ii=1:length(xx)
        ...
        if fixedDelay==99
            ...
            t_delay(ii)=mean(mean(fixedDelay));   % came from estGlobalDelay
            if fixedVp==99
                est_fb(ii)= xx(ii,3);
            end
        end
    ...
    for ii=1:numAzimuthalRegions*numRadialRegions
        ...
        if fixedDelay(ii)==99
            ...
            t_delay(ii)=fixedDelay(ii);   % came from estGlobalDelay
            if fixedVp==99
                est_fb(ii)= xx(ii,3);
            end
        end
        if fixedVp==99 & fixedDelay(ii)==99
            est_fb(ii)= xx(ii,4);
        end
...
if mean(fixedDelay(:))~=fixedDelay(1);  
  fixedDelay=0;
else
    fixedDelay=1;
end
