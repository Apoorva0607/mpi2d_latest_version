...
if T2star_corr_flag==0;  %%%%%  ( T2 star correction using two echos)
...
%      Echo1=curves(1,pd_end+5:end);
    
    Echo1=curves(1,:);
    scale1=mean(Echo1(1:10));
    
     curvefilename=strcat(outpath,'curves.study',int2str(seriesNum+1),'.slice',int2str(sliceNum),'.mat');
    load(curvefilename);
%      Echo2=curves(1,pd_end+5:end);
     Echo2=curves(1,:);
   
     Echo_2=Echo2;
    
    for i=1:length(Echo1)
        u(i)=log(Echo1(i)/Echo_2(i));
%         if(u(i)<=0&&i>2)
%           u(i)=u(i-1)+u(i-2);
%         end
        T2_star(i)=(TE2-TE1)/u(i);
        
        
    end
    
    scale_t2star1=exp(-TE1./T2_star);
    AIF_new1=Echo1(1:end)./scale_t2star1;
% end


       curves(1,:)=(AIF_new1); 
%         curves(1,pd_end+5:end)=(AIF_new1(pd_end+5:end));
end
% curvefilename=strcat(outpath,'curves.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.mat');
%     save(curvefilename, 'curves')
...
%                      [gdcurves T1 ]=conc_gd1(curves,Mz0,sliceNum,seriesNum,pd_end,data_info);  %%%%%% New mpi2d stg=3.4 added by Apoorva on 5/11/2017                      [gdcurves T1]=conc_gd_v2(curves,seriesNum,data_info);
