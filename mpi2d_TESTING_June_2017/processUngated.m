function processUngated(full_cinemri2,pd_frames,iseries, islice, preBolusFlag, saveflag,rangex_im,rangey_im,framesToSelect,ranget,flag_PD_end,pd_end)
...
numFrames=call_autoROI(full_cinemri2(:,:,1:end),islice, saveflag,iseries,flag_PD_end,pd_end);

call_sys_dias_modifyParFile(full_cinemri2,pd_frames,islice, iseries, isub_range,numFrames,rangey_im,rangex_im,framesToSelect,ranget,flag_PD_end,pd_end);

return

function call_sys_dias_modifyParFile(full_cinemri2,pd_frames,islice, iseries, isub_range,numFrames,rangey,rangex,framesToSelect,ranget,flag_PD_end,pd_end)
...
for isub=isub_range  
   ...
   if flag_PD_end==0;
   frames1=[1:pd_end peakloc];
   else
     frames1=[peakloc size(full_cinemri2,3)-9:size(full_cinemri2,3)-1];  
   end
 ...
 cinemri2_tmp=ANTS_DMBR1(buffer_cinemri,cinemri_curv_fit,mask,0.15,1,1);
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  for count=1:size(buffer_cinemri,3)
% mean_cinemri(:,:,count)=ones(size(buffer_cinemri,1),size(buffer_cinemri,2)).*mean(cinemri2_tmp,3);
%  end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 
 cinemri2=ANTS_DMBR1(buffer_cinemri,cinemri_curv_fit,mask,0.15,1,1);
 ...
 cinemri1_nup=zeros(size(cinemri2,1),size(cinemri2,2),size(cinemri2,3)+9);
 cinemri1_nup(:,:,end-8:end)=pd_frames(:,:,:);
 cinemri1_nup(:,:,1:end-9)=cinemri2; 
 end
% 
upsampleFactor=2;
clear cinemri1;
...
function numFrames=call_autoROI(full_cinemri2,islice,saveflag,iseries,flag_PD_end,pd_end)
if flag_PD_end==1
    fullcinemri1=full_cinemri2(:,:,1:end-10);
else
    fullcinemri1=full_cinemri2(:,:,1:end);
end
soss=fullcinemri1;

 try 
    load(['Output/RVLVpos.study',int2str(iseries),'.slice',int2str(islice),'.mat']);
 catch
...
hold on
lw=1.6;
%  new_systole=new_systole(find(new_systole));
%  new_diastole=new_diastole(find(new_diastole));
...
if saveflag
    peakloc=new_systole(find(new_systole>pd_end));
    save(fileout, 'peakloc')
end
...
if saveflag
    peakloc=new_diastole(find(new_diastole>pd_end));
    save(fileout, 'peakloc')
end
