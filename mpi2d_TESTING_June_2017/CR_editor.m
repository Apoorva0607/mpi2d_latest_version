...
global v
    ...
    if(~UpdateOnlyRecent || temp.datenum > v.OldTimeStamp || temp2.datenum > v.OldTimeStamp)
        ...
        if exist(['Output/shiftsMAN.study',num2str(v.seriesNum),'.slice',num2str(v.sliceNum),'.txt']) && ~(exist(['Output/shifts.study',num2str(v.seriesNum),'.slice',num2str(v.sliceNum),'.txt']))
            shifts=load(['Output/shiftsMAN.study',num2str(v.seriesNum),'.slice',num2str(v.sliceNum),'.txt'])
        else
            shifts=load(['Output/shifts.study',num2str(v.seriesNum),'.slice',num2str(v.sliceNum),'.txt']);  %EVRD 4/20/12
        end
...
%         else
        ...
        try    
            ...
            load(['timeStampSer' num2str(2.*floor((v.seriesNum-92)/2)) '.mat']);  %round  EVRD changed DEV
        end
...
function KtransData = loadMostRecentKtrans(seriesNum,sliceNum)
    clear KtransData
    KtransData.ktrans = zeros(6,1);
    
    %bmartin - changed this to work after Fermi Models have been run
    %          will still edit the 4.1 version????
   % files = dir(['Output/flowvalues.study' num2str(seriesNum) '.slice' num2str(sliceNum) '*']);
%     files = dir(['Output/flowvalues.study' num2str(seriesNum) '.slice' num2str(sliceNum) '.6.1_fixedDelay0.AIF_' num2str(seriesNum) '_' num2str(sliceNum) '_1.txt.full.txt']);
% 
...
global v
    ...
    for t=1:length(v.ShiftsData)
        myshifts(t,:) = v.ShiftsData(t).shift(1:2);  % added the (1:2) EVRD 4/20/12
    end
...
global v MasksChangedMutex
    ...
    v.blood= v.blood - mean(v.blood(5:9));
    curves=curves(2:end,:);  %skip AIF, gets shown only if AIF contour selected
    % now convert to deltaSI
    for i = 1:v.CurvesData.NAzimuthalRegions
          v.CurvesData.deltaSIcurves(i,ranget) = curves(i,ranget) - median(curves(i,5:9));
    end
...
%             shifts = zeros(v.st,2);
%             for t=1:v.st
%                 shifts(t,:) = v.ShiftsData(t).shift(1:2);  % EVRD 4/20/12
%                 shifts_tmp(t,:) = v.ShiftsData(t).shift(1:3);  % EVRD 4/20/12
%             end
%             %
            if size(v.ShiftsData(1).shift,2)==2  % EVRD 4/20/12
                shifts = zeros(v.st,2);
               for t=1:v.st
                  shifts(t,:) = v.ShiftsData(t).shift(1:2);  % EVRD 4/20/12
               end
               save(['Output/shiftsMAN.study' num2str(v.seriesNum) '.slice' num2str(v.sliceNum) '.txt'],'shifts','-ascii','-tabs');
                ...
                save(['Output/shifts.study' num2str(v.seriesNum) '.slice' num2str(v.sliceNum) '.txt'],'shifts_tmp','-ascii','-tabs');
            end
        else
...
global keyPressMutex
    ...
    elseif(strcmp(eventdata.Key,'uparrow') || ...
       ...
       % v.DCData.DCmri = circshift(v.DCData.DCmri,shift);
        ...
        v.ShiftsData(index).shift(1:2) = v.ShiftsData(index).shift(1:2) + shift;
    ...
    if(mouse(2) < size(v.masks,1) && mouse(1) < size(v.masks,2))
        ...
        if(strcmp(v.clickType,'normal') && ~v.bumping)
            ...
            if(norm(v.oldmouse - v.mouse) > 1)
                ...
                changedSomething = 1;
                v.oldmouse = v.mouse;
                v.ShiftsData(v.t).shift(1:2) = v.ShiftsData(v.t).shift(1:2) + diff;  % EVRD 4/20/12
