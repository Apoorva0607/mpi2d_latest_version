...
[X,Y,xCenter, yCenter, start_angle,bw1, bw3]= mpi_loadContours(img,sliceNum, studyNum,outpath,0);
nX     = length(X);


flagPixelwise=1;
numAzimuthalRegions=6; numRadialRegions=1;  % doesn't matter for pixelwise but needs to be defined 
clusterFlag=0;
...
imgParams=zeros(size(tmpImg,1),size(tmpImg,2),3);
...
imagesc(imgParams(:,:,1),[0 1]); axis('image')
save('image.mat','imgParams');
