...
cinemri2=ANTS_DMBR1(buffer_cinemri,cinemri_curv_fit,mask,0.1,0.5,0.5,1);
!rm -f *mhd
!rm -f *raw
!rm -f *gz
!rm -f *txt
...
T2star_corr_flag=0; % Change to 1 if using old conversion technique
...
if T2star_corr_flag==1;  %%%%% Used only if old conversion method needs to be used. ( T2 star correction using two echos)
    TE1=1.220;
    TE2=2.342;
    sin_PD=sin(2*pi/180);
...
%            gdcurves=mpi_si2gdconc_test2(Mz0,curves(:,pd_end+1:end),slice_sat_no(sliceNum),nor,pd_end);
       [gdcurves1 T1]=conc_gd1(curves,sliceNum,seriesNum,pd_end);  %%%%%% New mpi2d stg=3.4 added by Apoorva on 5/15/17
    
    % Old conversion method 
%      [gdcurves1 t1_value]=mpi_si2gdconc_hybrid1(Mz0,curves(:,pd_end+1+numSkip:end),slice_sat_no(sliceNum),sliceNum,seriesNum); 
  
  
    
    curvefilename=strcat(outpath,'gdcurves1.study',int2str(seriesNum),'.slice',int2str(sliceNum),'.mat');
    save(curvefilename, 'gdcurves1')
    mycolors = lines(numAzimuthalRegions);
    
    
    figure(seriesNum*10 + 2*sliceNum+1), clf, hold on, plot(gdcurves1(1,:),'R');
