function [ threeFlows ] = custom_AHA( ktrans,gat )
...
if gat==1
  tmp(:,1)=ktrans.sys(:,3);
   tmp(:,2)=ktrans.sys(:,2);   % so will be base to apex
   tmp(:,3)=ktrans.sys(:,1);
else

    tmp(:,1)=ktrans.dias(:,3);
   tmp(:,2)=ktrans.dias(:,2);   % so will be base to apex
   tmp(:,3)=ktrans.dias(:,1);
end  
   ...
   tmp=tmp(:);
   flows.sys(1:12)=tmp(1:12);
   flows.sys(13:16)=interp1(1:6, tmp(13:18),[1.5 3 4.5 6]);
...
threeFlows.sys=combineRegions(flows.sys)  % expects 18 regions AHA order
