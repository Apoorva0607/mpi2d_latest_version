...
%SEE ALSO EARLIER VERSION for 2009, in ED's mac in /Users/ Share../
%/matlab/readMeasData.. 
...
%   text = sprintf('%s%d',text,c-1);
 ...
 %    text = [text,'].flFactor = '];
  ...
  % try replace next section with one from readOne Coil... 
% %   text = [text,'].flFactor'];   % evrd 4/08  to read brain 12 coil
% %   t = strfind(info,text) + length(text);
% %   t=t+3;   % evrd to get back the space = space
% %  % keyboard
% %   %FFTCorrFactor(c) = sscanf(info(t:t+20),'%f');  % did not change 11/16/09
% %   %(well, did change but put it back, problem was usb drive was full, so
% %   %file not complete!
