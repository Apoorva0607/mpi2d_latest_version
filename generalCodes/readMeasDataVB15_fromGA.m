...
OFF = 0;
ON = 1;
nNavEK=0; %% changed this value to 1 (originally 0) to try to get timeStamps... NP 012910
...
if writeToFile == 1
  ...
  if transformToImageSpace == 0
     filenameOut = sprintf('%s_Kspace',filename_temp);
  else
...
for r=1:Ndr
  ...
  count = 0;
  countNavigator = 0;
  navigatorDataON = 0; %%% set to 1 (originally set to 0) in order to load and read timeStamps from raw data below NP012910
